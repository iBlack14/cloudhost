...
export function Sidebar({ isOpen = false, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-white border-r border-slate-200 fixed inset-y-0 left-0 z-50 p-5 flex flex-col shadow-xl shadow-slate-200/50 transform lg:transform-none transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="mb-6 flex justify-between items-center">
          <Link href="/" className="flex flex-col items-center gap-2 group text-center flex-1">
            ...
            <div className="flex flex-col items-center">
              <h2 className="text-md font-black tracking-tighter text-slate-900 uppercase italic leading-none">
                ODISEA <span className="text-[#00A3FF] not-italic">CLOUD</span>
              </h2>
              <span className="text-[7px] text-[#00A3FF] font-black uppercase tracking-[0.3em] mt-1.5">Portal de Usuario</span>
            </div>
          </Link>
          ...
          <button 
            onClick={onClose}
            className="lg:hidden w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 text-slate-400 hover:text-slate-600 flex items-center justify-center active:scale-95 transition-all"
          >
