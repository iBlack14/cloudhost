...
function ContextMenu({ menu, onClose, onEdit, onRename, onMove, onCopy, onDownload, onCompress, onExtract, onDelete, extracting, compressing }: {
  ...
  const item = (icon: string, label: string, action: () => void, color="text-slate-600", disabled=false) => (
    <button onClick={() => { if (!disabled) { action(); onClose(); } }} disabled={disabled}
      className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs font-semibold rounded-lg hover:bg-sky-50 transition-colors text-left disabled:opacity-40 ${color}`}>
  ...
  return (
    <div ref={ref} style={{ top: menu.y, left: menu.x }} onClick={(e) => e.stopPropagation()}
      className="fixed z-[550] bg-white border border-slate-200 rounded-xl shadow-2xl shadow-slate-900/15 p-1.5 min-w-[195px] animate-in zoom-in-95 fade-in duration-150">
      <p className="px-3 py-1.5 text-[9px] font-black text-slate-500 uppercase tracking-wider truncate max-w-[175px]">{menu.file.name}</p>
      <div className="border-t border-slate-100 my-1"/>
      {onEdit    && item("code",        "Editar contenido", onEdit,   "text-sky-500")}
      {            item("text_fields", "Cambiar nombre",    onRename, "text-slate-500")}
...
export default function FileManagerPage() {
  ...
  return (
    <div className="flex h-full bg-[#f5faff] text-slate-700 overflow-hidden">

      {/* ── Left sidebar ──────────────────────────────────────────────────────── */}
      <aside className="hidden md:flex w-56 flex-col bg-white border-r border-sky-100 shrink-0 overflow-hidden">
        {/* Actions */}
        <div className="p-3 border-b border-sky-100 space-y-1.5">
          <button onClick={()=>setNewFileOpen(true)}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-sky-50 hover:bg-sky-100 text-slate-700 hover:text-sky-700 text-xs font-semibold transition-all border border-sky-100">
            <span className="material-symbols-outlined text-[15px] text-sky-500">note_add</span>Nuevo archivo
          </button>
          <button onClick={handleCreateFolder}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-white hover:bg-sky-50 text-slate-700 hover:text-sky-700 text-xs font-semibold transition-all border border-slate-200">
            <span className="material-symbols-outlined text-[15px] text-sky-500">create_new_folder</span>Nueva carpeta
          </button>
        ...
        <div ref={dropZoneRef} className={`flex-1 flex flex-col overflow-hidden relative transition-all ${isDragging?"ring-2 ring-inset ring-[#00A3FF]/60":""}`}
          ...
          {/* Address bar */}
          <div className="px-3 py-2.5 border-b border-sky-100 flex items-center gap-1.5 bg-white shrink-0">
            <button onClick={navigateUp} disabled={currentPath==="/"} className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 hover:bg-sky-50 text-slate-500 hover:text-sky-600 transition-all flex items-center justify-center disabled:opacity-30 shrink-0" title="Subir un nivel">
              <span className="material-symbols-outlined text-[14px]">arrow_upward</span>
            </button>
            <button onClick={()=>refetch()} className="w-8 h-8 rounded-lg bg-slate-50 border border-slate-200 hover:bg-sky-50 text-slate-500 hover:text-sky-600 transition-all flex items-center justify-center shrink-0" title="Actualizar archivos">
              <span className={`material-symbols-outlined text-[14px] ${isLoading?"animate-spin":""}`}>refresh</span>
            </button>
            <form onSubmit={handlePathSubmit} className="flex-1 flex items-center bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 gap-1.5 min-w-0 focus-within:border-sky-300 focus-within:ring-2 focus-within:ring-sky-100">
            ...
            <div className="relative flex items-center w-36 shrink-0">
              ...
              <input type="text" value={searchQuery} onChange={(e)=>setSearchQuery(e.target.value)} placeholder="Buscar..."
                className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-7 pr-5 py-1.5 text-xs text-slate-700 focus:outline-none focus:border-sky-300 focus:ring-2 focus:ring-sky-100 transition-all placeholder:text-slate-400"/>
          ...
          {/* Sort + count bar */}
          <div className="px-3 py-1.5 border-b border-sky-100 bg-[#f8fbff] flex items-center gap-2 shrink-0">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-2">
                ...
                {displayFiles.map((file)=>(
                  <div key={file.path} onClick={()=>file.isDirectory&&navigateTo(file.name)} onContextMenu={(e)=>openCtx(e,file)}
                    ...
                    {renameTarget!==file.path&&(
                      <div className="absolute bottom-1 right-1 hidden md:flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/95 backdrop-blur-sm p-1 rounded-lg border border-slate-200 shadow-lg" onClick={(e)=>e.stopPropagation()}>
                        {!file.isDirectory&&isTextFile(file.name)&&<button onClick={()=>openEditor(file.path,file.name)} className="w-6 h-6 rounded-md bg-sky-50 text-sky-600 hover:bg-sky-100 flex items-center justify-center" title="Editar contenido"><span className="material-symbols-outlined text-[14px]">code</span></button>}
                  ...
                  <tbody className="divide-y divide-white/[0.03]">
                    ...
                    {displayFiles.map((file)=>(
                      <tr key={file.path} onContextMenu={(e)=>openCtx(e,file)}
                        className={`transition-colors group border-l-2 ${selectedFiles.has(file.path)?"border-l-sky-400 bg-sky-50":"border-transparent hover:bg-slate-50 hover:border-l-sky-200"}`}>
                        ...
                        <td className="px-3 py-1.5 text-right" onClick={(e)=>e.stopPropagation()}>
                          <div className="hidden md:flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={()=>startRename(file)} className="h-7 px-2 rounded-md bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all flex items-center gap-1" title="Cambiar nombre"><span className="material-symbols-outlined text-[14px]">text_fields</span><span className="text-[9px] font-bold">Nombre</span></button>
          ...
          {/* Status bar */}
          <div className="px-4 py-1.5 border-t border-sky-100 bg-white flex items-center justify-between gap-4 shrink-0">
            <p className="text-[9px] font-mono text-slate-500 truncate">{currentPath}</p>
      ...
      {/* ── Editor Modal ──────────────────────────────────────────────────────── */}
      {editorOpen&&(
        <div className="fixed inset-0 z-[200] flex items-center justify-center sm:p-5 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-slate-900/30 backdrop-blur-sm" onClick={()=>!editorSaving&&setEditorOpen(false)}/>
          <div className="relative z-10 w-full h-full sm:h-[92vh] sm:max-w-7xl flex flex-col bg-white border border-sky-100 sm:rounded-2xl shadow-2xl shadow-slate-900/20 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-sky-100 bg-white">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                <span className="hidden lg:block text-[9px] text-slate-500 font-mono bg-slate-50 border border-slate-200 px-2 py-1 rounded-md">Ctrl+S para guardar · Tab = 2 espacios</span>
                ...
                <button onClick={()=>setEditorOpen(false)} disabled={editorSaving} className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center rounded-xl bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-all" title="Cerrar editor">
