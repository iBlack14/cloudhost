...
export default function SslCertificatesPage() {
  ...
  const autoSslMutation = useMutation({
    mutationFn: async ({ domainId, renew }: { domainId: string; renew: boolean }) => {
      ...
      return { domainId, status: data.data as SslStatus, message: data.message as string };
    },
    onSuccess: ({ domainId, status, message }) => {
      // Update only this domain's SSL status — don't refresh all rows
      queryClient.setQueryData<Record<string, SslStatus>>(
        ["odin", "ssl_statuses", domains.map(d => d.id).join(",")],
        (prev) => ({ ...(prev ?? {}), [domainId]: status })
      );
      queryClient.invalidateQueries({ queryKey: ["odin", "domains"] });
      alert(message ?? "Certificado SSL procesado correctamente.");
      setActiveDomainId(null);
    },
    onError: (err: Error) => {
      alert(`Error al emitir SSL: ${err.message}`);
      setActiveDomainId(null);
    },
  });
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 pb-10">
         <div className="space-y-1.5">
           ...
           <p className="text-slate-500 text-sm font-medium mt-2">
             Cada dominio tiene su certificado independiente. Emitir o regenerar uno no afecta a los demás.
           </p>
         </div>
      </header>
              ...
              <table className="w-full text-left">
                 ...
                 <tbody>
                    {domains.map((dom) => {
                      ...
                      const isThisDomainBusy = activeDomainId === dom.id && autoSslMutation.isPending;
                        ...
                        <tr key={dom.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                           ...
                           <td className="p-5 text-right pr-8">
                               <div className="flex items-center justify-end gap-2">
                                 ...
                                 <button
                                   onClick={() => autoSslMutation.mutate({ domainId: dom.id, renew: hasSsl })}
                                 ...
                                 >
                                    <span className={`material-symbols-outlined text-[16px] ${isThisDomainBusy ? "animate-spin" : ""}`}>
                                      {isThisDomainBusy ? "progress_activity" : "sync"}
                                    </span>
                                    {isThisDomainBusy ? "Procesando..." : hasSsl ? "Regenerar" : "Auto-SSL"}
                                 </button>
                               </div>
                            </td>
                        </tr>
