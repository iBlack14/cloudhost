...
export async function POST(request: Request, { params }: { params: { messageId: string } }) {
  ...
  try {
    ...
    const data = await apiFetch<{ success: boolean }>(`/mail/messages/${params.messageId}/move`, {
      method: "POST",
      token,
    ...
    return NextResponse.json(
      {
        success: false,
        error: { code: "MAIL_MOVE_FAILED", message: error instanceof Error ? error.message : "No se pudo mover el mensaje" }
      },
      { status: 400 }
    );
  }
