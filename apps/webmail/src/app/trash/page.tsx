  ...
  return (
    ...
    >
      <div className="flex h-full w-full overflow-hidden flex-col bg-white">
        ...
        <div className="bg-amber-50/50 border-b border-amber-100/50 px-6 py-2.5 text-center flex items-center justify-center gap-3">
           <span className="material-symbols-outlined text-amber-400 text-[18px]">info</span>
           <span className="text-xs font-medium text-slate-500">Los mensajes que lleven más de 30 días en la Papelera se borrarán automáticamente.</span>
           <button className="text-xs font-semibold text-[#00A3FF] hover:underline ml-2">Vaciar papelera</button>
        </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar">
                 {loading ? (
                   <div className="p-24 flex flex-col items-center gap-4 opacity-30">
                     ...
                     <span className="text-xs font-medium text-slate-400">Recuperando Papelera...</span>
                   </div>
                 ) : messages.length === 0 ? (
                   <div className="p-32 text-center flex flex-col items-center opacity-50">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mb-5 border border-slate-100">
                         <span className="material-symbols-outlined text-3xl text-slate-300">delete</span>
                      </div>
                   ...
                   messages.map((message) => (
                     <div
                       ...
                       className={`
                         ...
                         ${selectedId === message.id ? "bg-slate-100/80 border-l-2 border-l-slate-400" : "hover:bg-slate-50/70 border-l-2 border-l-transparent"}
                         bg-white
                       `}
                     >
                       ...
                       {selectedId ? (
                         <div className="flex-1 min-w-0">
                           <div className="flex items-center justify-between mb-0.5">
                             <span className="text-[13px] truncate font-normal text-slate-400 italic">{message.from}</span>
                             <span className="text-[11px] font-medium text-slate-400 shrink-0 ml-2">{message.receivedAt}</span>
                           </div>
                           <div className="text-[13px] truncate font-medium text-slate-500 line-through decoration-slate-300">{message.subject}</div>
                           <div className="text-xs text-slate-400 truncate mt-0.5 font-normal">{message.preview}</div>
                         </div>
                       ) : (
                         <>
                           <div className="w-48 shrink-0 text-[13px] truncate mr-4 text-slate-400 font-normal italic">
                              {message.from}
                           </div>

                           <div className="flex-1 min-w-0 flex items-baseline gap-1.5">
                              <span className="text-[13px] truncate font-medium text-slate-500 line-through decoration-slate-300">
                                {message.subject}
                              </span>
    ...
    <div className="flex flex-col h-full bg-white">
       ...
       <div className="flex-1 overflow-y-auto p-8 lg:p-10 custom-scrollbar">
          <div className="max-w-3xl mx-auto space-y-8">
             {/* Trash info banner */}
             <div className="bg-slate-50 border border-slate-100 px-5 py-4 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-3 text-slate-500">
                   <span className="material-symbols-outlined text-[20px]">info</span>
                   <span className="text-xs font-medium">Este mensaje está en la papelera</span>
                </div>
                <button onClick={onRestore} className="bg-slate-800 text-white text-xs font-semibold px-5 py-2 rounded-lg hover:bg-slate-700 transition-all">Restaurar</button>
             </div>

             <h2 className="text-2xl font-bold text-slate-400 leading-snug line-through decoration-slate-200">{summary.subject}</h2>

             <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-full bg-slate-200 text-slate-400 flex items-center justify-center font-semibold text-sm">
                   {summary.from?.charAt(0).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                   <div className="flex items-center justify-between mb-0.5">
                      <div className="flex items-center gap-2 min-w-0">
                         <span className="text-sm font-semibold text-slate-500">{summary.from}</span>
                         <span className="text-xs text-slate-300 truncate">&lt;{summary.fromAddress}&gt;</span>
                      </div>
                      <div className="text-xs font-medium text-slate-400 shrink-0 ml-4">{summary.receivedAt}</div>
                   </div>
                   <div className="text-xs text-slate-400">
                      para <span className="text-slate-500 font-medium">{me?.address}</span>
                   </div>
                </div>
             </div>

             <div className="text-slate-400 text-[15px] leading-[1.8] whitespace-pre-line border-t border-slate-100 pt-8">
                {message?.body ? (
                   <div className="animate-in fade-in duration-500 opacity-70">{message.body}</div>
                ) : (
                  <div className="flex items-center gap-3 text-slate-200">
                    <div className="w-4 h-4 border-2 border-slate-200 border-t-slate-400 rounded-full animate-spin"></div>
                    <span className="text-xs font-medium text-slate-400">Cargando mensaje eliminado...</span>
                  </div>
                )}
             </div>
          </div>
